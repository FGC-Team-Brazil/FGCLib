 ...
 *
 *   // In execute():
 *   motor.setPower(pidController.calculate(TARGET, motor.getCurrentPosition()));
 *
 ...
 *
 * ── Velocity control (launchers, shooters) ────────────────────────────
 *
 *   // In execute():
 *   pidController.runVelocity(motor, TARGET_TICKS_PER_SEC);
 */
public class PIDController {
     ...
     * The motor accelerates smoothly to maxVelocity, cruises, then decelerates.
     ...
     * Robot behavior stays consistent as the battery drains during a match.
     * Call this in initialize().
     *
    ...
    /**
     * Calculates the motor power to reach the target position.
     * Call this every loop iteration in execute().
     *
     * If motion profile is enabled, the target is smoothed automatically.
     * If voltage compensation is enabled, output is scaled automatically.
     *
    ...
    /**
     * Sets power directly on a motor using the last calculated output.
     * Call calculate() first, then setPowerMotor() to apply.
     *
    ...
    /**
     * [Velocity control] Measures speed from encoder and applies PID automatically.
     * Call every loop iteration in execute().
     *
     * Uses its own isolated PID state — safe to use alongside calculate()
     * in the same subsystem without state corruption.
     *
    ...
    /**
     * Resets all PID state — position and velocity.
     * Call when re-enabling after a pause or changing targets abruptly.
     */
    public void reset() {
        integralSum      = 0;
        lastError        = 0;
    ...
    /**
     * Tolerance in ticks or degrees.
     * Within this range the controller considers itself at target and holds with kF only.
     * Default is 0 — set this to a small value (e.g. 10 ticks) for atTarget() to work.
     */
